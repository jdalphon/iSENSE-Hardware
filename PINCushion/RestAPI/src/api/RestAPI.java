...
import exceptions.columnMatchException;
...
import objects.ExperimentField;
import objects.Item;
...
public class RestAPI {
    ...
    public ArrayList<SessionData> sessiondata(String sessions) {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                ...
                temp.FieldsJSON = current.getJSONArray("feilds");
    ...
    private boolean putSessionData(int sid, String eid, JSONArray dataJSON) {
    ...
    private boolean updateSessionData(int sid, String eid, JSONArray dataJSON) {
    ...
    private String makeRequest(String target) throws Exception {
    ...
    public boolean addToSession(int expID, int sesID, String[] headers,
            ArrayList<String[]> records) throws loginException, columnMatchException {
        ...
        if (isLoggedIn()) {
            if (isSessionOwner(sesID)) {
                ...
                if (data != null) {
                    Boolean result = putSessionData(sesID, expID + "", data);
    ...
    private void fieldMatch(int expID, String[] fields) throws columnMatchException {
        
        
        if (!columnsMatched) {
            //Pull the experiment ID from the form, and return the field names
            //(sensors) required by the experiment
            ArrayList<ExperimentField> experimentFields = getExperimentFields(expID);
    ...
    public boolean isSessionOwner(int sessID) {
        if (!sessionVerified || (sessID != sessionID)) {
            ArrayList<SessionData> sessionData = sessiondata(sessID + "");
        ...
        public JSONArray createDataJSON(int expID, String[] fields, ArrayList<String[]> records) throws columnMatchException {
